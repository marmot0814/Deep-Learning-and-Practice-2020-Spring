...
class DQN:
    ...
    def _update_behavior_network(self, gamma):
        ...
        with torch.no_grad():
            action = self._behavior_net.forward(next_state).argmax(dim=1).view(-1, 1)
...
def train(args, env, agent, writer):
    ...
    total_steps, epsilon = 0, args.eps_max
    ewma_reward = 0
...
def main():
    ...
    parser.add_argument('-m', '--model', default='doubledqn.pth')
    parser.add_argument('--logdir', default='log/doubledqn')
    # train
    ...
    parser.add_argument('--eps_max', default=1, type=float)
