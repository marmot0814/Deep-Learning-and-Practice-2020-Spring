...
from dataset.displayer import line_plot, decision_region
...
class Network:
    ...
    def __init__(self, structure=[]):
        ...
        self.acc_ax = self.fig.add_subplot(gs[0, 0:2])
        self.err_ax = self.fig.add_subplot(gs[1, 0:2])
        self.res_ax = self.fig.add_subplot(gs[0:2, 2:4])
    ...
    def train(self, X, Y, epochs = 10):
        ...
        for epoch in range(epochs):
            ...
            line_plot(self.err_ax, self.errors, f"error({self.loss.name()})", [0, max(self.errors)])
            if epoch % 1 == 0:
